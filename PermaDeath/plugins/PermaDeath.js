!function() {
	...
	//Had to make a separate fonction just to have an more aesthetic Game Over screen in terms of timing
	function PermaDeathMod () {
		StorageManager.remove(DataManager.lastAccessedSavefileId());
		SceneManager.goto(Scene_OmoriTitleScreen);
	}

	//Had to import the entire function because my modification can't be made in advance (the modification are line 73 and 74)
		...
		if (anim.active) {
		  ...
		  switch (anim.phase) {
			...
			if (this._isFinalBattle  && this._finalBattlePhase >= 5) {
			  ...
			  //PERMADEATH MODIFICATION-----------------------------------------------------------------------------------------------------------------------------------------------------------
			  setTimeout(PermaDeathMod, 6000); //If we're dead and it's not the scripted death during the last fight, wait 6 seconds and go back to the title screen
